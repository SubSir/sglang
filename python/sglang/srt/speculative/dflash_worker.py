...
class DFlashWorker:
    ...
    def _prepare_for_speculative_decoding(
        ...
        if _can_use_piecewise_graph:
            ...
            if cuda_graph_shapes_t is None:
                if _can_use_piecewise_graph and hasattr(
                    _pcg_runner, "_get_dflash_capture_tokens_for_bs"
                ):
                    cuda_graph_shapes = list(
                        _pcg_runner._get_dflash_capture_tokens_for_bs(bs)
                    )
                else:
            ...
            shape_idx = torch.searchsorted(cuda_graph_shapes_t, total_verify_tokens_t)
        ...
        if (
            ...
            self._verify_tokens_flat_buf = torch.empty(
                (new_cap,), dtype=torch.long, device=self.device
            )
    ...
    def forward_batch_generation(
        ...
        **kwargs,
        ...
        model_worker_batch = batch.get_model_worker_batch()
        assert (
            model_worker_batch.forward_mode.is_target_verify()
            or model_worker_batch.forward_mode == ForwardMode.DFLASH_VERIFY
        )
